...
| `category` | **CAN** | An optional user-specified category label for a finetuned model. A model trained on a specific person would be named after that person, but categorized as simply `Person`. Model listings can use this key when present to organize and allow easy filtering by type. When in doubt on what category to use, the model maker is encouraged to look at the category label on other similar models, or choose a new label if they're the first to make a model in a category. | `Person`, `Object`, `Style` |
